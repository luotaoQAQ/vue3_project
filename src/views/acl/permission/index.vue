<template>
  <div>
    ...
    >
      ...
      <template #footer>
        <el-button type="primary" plain @click="cancelAddOrUpdatePermission">
          取消
        </el-button>
        <el-button type="primary" @click="saveAddOrUpdatePermission">
          确定
        </el-button>
      </template>
    </el-dialog>
...
async function saveAddOrUpdatePermission() {
  ...
  if (result.code === 200) {
    ElMessage({
      type: 'success',
      message: permissionParams.id ? '修改成功' : '添加成功',
    })
    dialogVisible.value = false
    getPermissionList()
  } else {
    ElMessage({
      type: 'error',
      message: permissionParams.id ? '修改失败' : '添加失败',
    })
  }
}
