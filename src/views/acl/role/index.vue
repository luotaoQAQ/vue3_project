<template>
  <div>
    <el-card style="margin-bottom: 10px;">
      <el-form inline class="form-flex">
        <el-form-item label="角色搜索">
          <el-input placeholder="请输入用户角色关键字" v-model="searchRoleName"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="search" :disabled="!searchRoleName.trim()">搜索</el-button>
          <el-button @click="settingStore.isRefresh = !settingStore.isRefresh">重置</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card>
      ...
      <el-table border style="margin: 10px 0;" :data="roleArr">
        ...
        <el-table-column label="操作" width="300" align="center" fixed="right">
          <template #default="{ row }">
            <el-button type="primary" icon="User" size="small" @click="assignRole(row)">分配权限</el-button>
            <el-button type="warning" icon="Edit" size="small" @click="updateRole(row)">编辑</el-button>
            <el-popconfirm :title="`你确定要删除${row.roleName}吗?`" width="200" icon="Warning" @confirm="removeRole(row)">
              <template #reference>
      ...
      <el-pagination v-model:current-page="pageNo" v-model:page-size="limit" :page-sizes="[10, 20, 30, 40]" size="small"
        :background="true" layout="prev, pager, next, jumper, ->, sizes, total" :total="total" @change="getRoleList">
      </el-pagination>

      <!-- 新增或修改角色 -->
      <el-dialog title="添加角色" v-model="dialogVisible" @close="clearDataAndValidation">
        <el-form style="width: 80%;" :model="roleParams" :rules="rules" ref="formRef">
          <el-form-item label="角色名称" prop="roleName">
            <el-input placeholder="请输入角色名称" v-model="roleParams.roleName"></el-input>
          </el-form-item>
        </el-form>
        <template #footer>
          <el-button type="primary" plain @click="cancelAddOrUpdateRole">取消</el-button>
          <el-button type="primary" @click="saveAddOrUpdateRole">确定</el-button>
        </template>
      </el-dialog>
...
import { reqAddOrUpdateRole, reqAssignRolePermission, reqPermissionByRoleId, reqRemoveRole, reqRolePageList } from '@/api/acl/role';
...
async function getRoleList() {
  const result = await reqRolePageList(pageNo.value, limit.value, searchRoleName.value)
...
async function saveAssignRole() {
  ...
  const permissionId = [...treeRef.value.getHalfCheckedKeys(), ...treeRef.value.getCheckedKeys()]
