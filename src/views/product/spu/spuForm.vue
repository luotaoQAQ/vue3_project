...
function toLook(row: spuSaleAttr) {
  ...
  // 属性值不能重复
  let repeat = row.spuSaleAttrValueList.findIndex(
...
async function initSpuData(spu?: spuData) {
  ...
  if (spu) {
    ...
    const result2: spuImageResponseData = await reqSpuImageList(
      spu.id as number,
    )
    const result3: spuSaleAttrResponseData = await reqSpuSaleAttrList(
      spu.id as number,
    )
