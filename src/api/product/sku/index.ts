...
export const reqSkuPageList = (page: number, limit: number) =>
  request<any, skuPageListResponseData>(
    API.GET_SKU_PAGELIST_URL + `${page}/${limit}`,
  )

// 上架SKU接口
export const reqSkuOnSale = (skuId: number) =>
  request.get<any, any>(API.SKU_ONSALE_URL + skuId)

// 下架SKU接口
export const reqSkuCancelSale = (skuId: number) =>
  request.get<any, any>(API.SKU_CANCELSALE_URL + skuId)

// 获取SKU详情接口
export const reqSkuDetail = (skuId: number) =>
  request.get<any, skuDetailResponseData>(API.GET_SKU_DETAIL_URL + skuId)
