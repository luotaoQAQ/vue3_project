...
import type { menuResponseData, roleData, rolePageListResponseData } from "./type";
...
// 获取角色分页列表
export const reqRolePageList = (page: number, limit: number, roleName: string) => request.get<any, rolePageListResponseData>(API.GET_ROLE_PAGELIST_URL+`${page}/${limit}`, {params: {roleName}})
...
// 根据角色获取菜单接口
export const reqPermissionByRoleId = (roleId: number) => request.get<any, menuResponseData>(API.GET_PERMISSION_URL+roleId)

// 给角色分配权限
export const reqAssignRolePermission = (roleId: number, permissionId: number[]) => request.post<any, any>(API.ASSIGN_PERMISSION_ROLE_URL, undefined, {
  params: {
    roleId,
